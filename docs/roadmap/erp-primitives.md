...
| Typed actions and events | Inventory and Sales publish PUBLIC v1 action/event pairs; Accounting publishes its owner-controlled PUBLIC revenue event | `READY` for selected Level 3 slices | Accounting revenue action release remains gated on owner-verifiable amount provenance; future Process Studio owns aggregation/release |
