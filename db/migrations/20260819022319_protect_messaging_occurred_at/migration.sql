...
-- rationale: keep durable event occurrence time stable for audit ordering and idempotent replay
...
BEGIN
  IF OLD.tenant_id IS DISTINCT FROM NEW.tenant_id OR
    ...
    RAISE EXCEPTION USING
      ...
      MESSAGE = 'event outbox durable identity is immutable';
  END IF;
  RETURN NEW;
END;
$$;
...
BEFORE UPDATE OF tenant_id, id, event_type, event_version, idempotency_key, occurred_at
